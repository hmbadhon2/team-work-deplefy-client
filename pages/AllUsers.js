...
const AllUsers = () => {
    ...
    const { data: users = [], isLoading, refetch } = useQuery({
        queryKey: ['users'],
        queryFn: async () => {
            const res = await fetch('https://deplefy-server-mocha.vercel.app/users');
    ...
    const handleMakeAdmin = id => {
        fetch(`https://deplefy-server-mocha.vercel.app/users/admin/${id}`, {
            method: 'PUT',
            // headers: {
    ...
    const handleUserStatusUpdate = (id) => {
        fetch(`https://deplefy-server-mocha.vercel.app/users/${id}`, {
            method: 'PATCH',
            headers: {
    ...
    const handleUserDelete = (user) => {
        fetch(`https://deplefy-server-mocha.vercel.app/users/${user._id}`, {
            method: 'DELETE',
            headers: {
                authorization: `bearer ${localStorage.getItem('accessToken')}`
            }
